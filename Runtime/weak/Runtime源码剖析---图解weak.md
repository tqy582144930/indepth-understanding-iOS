...
- 最后我们留下了一个悬念，如何把指向当前的弱指针置为nil？我会在后面接着着给大家讲解
...
1. 删除旧的` weak` 指针时, 会使用原本指向的对象的地址来查找对应的` weak_entry_t`, 从中删除这个 `weak`指针. 如果删除之后`weak`指针数组为空, 则销毁这个 `weak_entry_t`, 原有位置置空, 原本被指向对象的 `isa` 指针的` weak` 引用标记位 0.
2. 添加新的`weak` 指针时, 如果查找到对应的` weak_entry_t`, 则将 `weak` 指针插入到 `referrers` 数组中. 如果没找到则创建一个 `weak_entry_t` 配置好后插入 `weak_table_t` 的数组中.